    ...
    return (
      <main className="mx-auto flex min-h-screen w-full max-w-6xl flex-col justify-center px-6 py-12 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          <section className="surface-card rounded-[2rem] p-8 lg:p-12">
            <div className="mb-8 inline-flex items-center gap-3 rounded-full bg-[color:var(--color-indigo-light)] px-4 py-2 text-sm font-semibold text-[color:var(--color-indigo)]">
              <span className="h-2.5 w-2.5 rounded-full bg-[color:var(--color-green)]" />
              Trainee testing platform
            </div>

            <h1 className="max-w-2xl text-5xl font-extrabold tracking-tight text-[color:var(--color-gray-900)]">
              Timed assessments for ongoing ighub cohorts.
            </h1>
            <p className="mt-5 max-w-2xl text-lg leading-8 text-[color:var(--color-gray-700)]">
              Trainees sign in by magic link, complete a randomized test, and get their
              score immediately. Admins manage question banks, results, retakes, and CSV
              exports from one interface.
            </p>
          ...
          >
            <h2 className="text-3xl font-bold text-[color:var(--color-gray-900)]">
            ...
            <div className="mt-6 space-y-4">
              <article className="rounded-[1.75rem] bg-[color:var(--color-gray-100)] p-5">
                ...
                <p className="mt-2 text-sm leading-7 text-[color:var(--color-gray-700)]">
                  No passwords to manage. The auth callback restores the session directly in
                  the app.
                </p>
              </article>
              <article className="rounded-[1.75rem] bg-[color:var(--color-gray-100)] p-5">
                ...
                <p className="mt-2 text-sm leading-7 text-[color:var(--color-gray-700)]">
                  Each attempt draws a locked subset from the bank, so trainees do not all
              ...
              <article className="rounded-[1.75rem] bg-[color:var(--color-gray-100)] p-5">
                ...
                <p className="mt-2 text-sm leading-7 text-[color:var(--color-gray-700)]">
                  Track completions, adjust retakes, and export session-level and answer-level
                  data to CSV.
                </p>
              </article>
            </div>
    ...
    <main className="mx-auto w-full max-w-6xl px-6 py-10 lg:px-8">
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <div className="flex flex-wrap items-center gap-3">
              <span className={`status-pill ${getRoleClasses(profile?.role ?? "trainee")}`}>
            ...
            <h1 className="mt-4 text-4xl font-bold text-[color:var(--color-gray-900)]">
              Welcome back
            </h1>
            <p className="mt-3 max-w-3xl text-sm leading-7 text-[color:var(--color-gray-700)]">
              Start a new active test, resume an assessment already in progress, or review
              the results from completed attempts.
            </p>
          </div>
      ...
      <section className="mt-8 grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="space-y-4">
          {(activeTests ?? []).map((test) => {
            ...
            const canStartNewAttempt =
              !latestSession || latestSession.retakes_remaining > 0 || isLiveSession;

            return (
              <article key={test.id} className="surface-card rounded-[2rem] p-6">
                <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[color:var(--color-purple)]">
                      Active assessment
                    </p>
                    <h2 className="mt-2 text-2xl font-bold text-[color:var(--color-gray-900)]">
                      {test.title}
                    </h2>
                    <p className="mt-3 text-sm leading-7 text-[color:var(--color-gray-700)]">
                      {test.questions_per_attempt} randomized questions · {test.time_limit_mins}{" "}
                      minute limit
                    </p>
                    {latestSession ? (
                      <p className="mt-3 text-sm text-[color:var(--color-gray-600)]">
                        Latest attempt: {latestSession.attempt_number} · retakes remaining:{" "}
                        {latestSession.retakes_remaining}
                      </p>
                    ) : (
                      <p className="mt-3 text-sm text-[color:var(--color-gray-600)]">
                  ...
                  {isLiveSession ? (
                    ...
                    <form action={startTestSessionAction}>
                      ...
                      <SubmitButton
                        ...
                        className="primary-button inline-flex items-center justify-center px-5 py-3 text-sm"
                      >
                        {latestSession ? "Start next attempt" : "Start assessment"}
                      </SubmitButton>
                    </form>
                  )}
                </div>
          ...
          {!activeTests?.length ? (
            <section className="surface-card rounded-[2rem] p-8 text-sm text-[color:var(--color-gray-700)]">
              No active tests are available yet. Check back when your cohort assessment has
              been published.
            </section>
          ) : null}
        </div>
        ...
        <section className="surface-card rounded-[2rem] p-6">
          <h2 className="text-2xl font-bold text-[color:var(--color-gray-900)]">
            Attempt history
          </h2>
          <div className="mt-5 space-y-3">
            {(sessions ?? []).length ? (
              (sessions ?? []).map((session) => {
                const href =
                  session.status === "in_progress" ? `/test/${session.id}` : `/results/${session.id}`;

                return (
                  <Link
                    key={session.id}
                    href={href}
                    className="block rounded-[1.5rem] border border-[color:var(--color-gray-300)] bg-white px-4 py-4 transition hover:border-[color:var(--color-indigo)]"
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <p className="font-semibold text-[color:var(--color-gray-900)]">
                          {(activeTests ?? []).find((test) => test.id === session.test_id)?.title ??
                            "Assessment"}
                        </p>
                        <p className="mt-1 text-sm text-[color:var(--color-gray-500)]">
                          Attempt {session.attempt_number} · {formatDateTime(session.started_at)}
                        </p>
                      </div>
                    ...
                    <p className="mt-3 text-sm text-[color:var(--color-gray-700)]">
                      Score: {session.score ?? 0}/{session.total_questions ?? "—"}
                    </p>
                  </Link>
                );
              })
