...
export default async function ResultsPage({ params }: ResultsPageProps) {
  ...
  const answeredCount = experience.questions.filter(
    (question) => question.selectedAnswer,
  ).length;
  const score = experience.session.score ?? 0;
  const totalQuestions =
    experience.session.total_questions ?? experience.questions.length;
    ...
    <main className="mx-auto w-full max-w-5xl px-6 py-10 lg:px-8">
      <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex gap-3">
          <span
            className={`status-pill ${getStatusClasses(experience.session.status)}`}
          >
      ...
      <section className="surface-card mt-8 rounded-4xl p-8">
        <div className="grid gap-6 md:grid-cols-3">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.18em] text-gray-500">
              Score
            </p>
            <p className="mt-2 text-5xl font-display text-(--color-indigo)">
              {score}
        ...
        {experience.questions.map((question) => {
          const answerState =
            question.isCorrect === true
              ? {
                  label: "Correct",
                  classes: "bg-(--color-green) text-white",
                }
              : question.isCorrect === false
                ? {
                    label: "Incorrect",
                    classes: "bg-(--color-orange) text-white",
                  }
                : {
                    label: "Unanswered",
                    classes: "bg-(--color-indigo-light) text-(--color-indigo)",
                  };
          ...
          return (
            ...
            >
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                ...
                <span className={`status-pill ${answerState.classes}`}>
                  {answerState.label}
                </span>
              </div>

              <div className="mt-5 rounded-3xl bg-gray-100 px-4 py-4">
